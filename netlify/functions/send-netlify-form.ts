...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // Submit to Netlify Forms - use the current site URL
    const siteUrl = process.env.URL || "https://frescoapp.netlify.app";
    ...
    if (orderData.customerIntercom)
      console.log(`🔔 Citofono: ${orderData.customerIntercom}`);
    console.log(`🛍️ Prodotti:\n${itemsList}`);
    console.log(`💰 Totale: €${totalAmount}`);
    if (orderData.deliveryInstructions)
      console.log(`📦 Istruzioni: ${orderData.deliveryInstructions}`);
